...
# Fully rendered scene class for the headset in the middle of the viewport, and the floor headsets on the bottom of the viewport
class HeadsetSimulation:
    ...
    def create_floor_headsets(self):
        ...
        headset_radius = 1.0 # Used for collision sphere
        safety_margin = 1.0
    ...
    def render_model(self, model_obj, floor=False):
        ...
        # Transform vertices and calculate normals
...
def main():
    # Run simulation with alpha=1.0 (pure gyroscope)
    print("\n=== Running simulation with alpha=1.0 (pure gyroscope) ===\n")
    
    # Create a custom class that overrides needed methods
    class GyroSimulation(HeadsetSimulation):
        def load_imu_data(self):
            path = "./IMUdata.csv"
            self.sensor_data = None
            self.dr_filter = None
            ...
            try:
                if os.path.exists(path):
                    ...
                    # Create filter with alpha=1.0 (pure gyroscope)
                    print("Creating DeadReckoningFilter with alpha=1.0 (pure gyroscope)")
                    self.dr_filter = DeadReckoningFilter(alpha=1.0)
                    if self.sensor_data and len(self.sensor_data) > 0:
                        self.dr_filter.calibrate(self.sensor_data[:min(100, len(self.sensor_data))])
                        self.current_data_index = 0
            except Exception as e:
        ...
        def stop_recording(self):
            ...
            video_path = self.video_recorder.save_video("headset_simulation_gyro_only.mp4")
            print(f"Video saved to: {video_path}")
    
    sim1 = GyroSimulation()
    sim1.run()
    
    # Run with alpha=0.9 (sensor fusion)
    print("\n=== Running simulation with alpha=0.9 (sensor fusion) ===\n")
    
    class FusionSimulation(HeadsetSimulation):
        def load_imu_data(self):
            path = "./IMUdata.csv"
            self.sensor_data = None
            self.dr_filter = None
            ...
            try:
                if os.path.exists(path):
                    ...
                    # Create filter with alpha=0.9 (sensor fusion)
                    print("Creating DeadReckoningFilter with alpha=0.9 (sensor fusion)")
        ...
        def stop_recording(self):
            ...
            # Save with custom name
            video_path = self.video_recorder.save_video("headset_simulation_sensor_fusion.mp4")
            print(f"Video saved to: {video_path}")
    
    sim2 = FusionSimulation()
    sim2.run()
