...
class HeadsetSimulation:
    def __init__(self, width=800, height=600):
        ...
        # Physics settings - Significantly reduced friction coefficient for much faster deceleration
        self.friction_coefficient = 0.7  # Changed from 0.95 to 0.7 (much more friction)
        self.accumulator = 0
    ...
    def update_floor_physics(self, dt):
        ...
        boundary = {
            ...
            'min_z': -40.0,
            'max_z': 0.0,
            'bounce_factor': 0.5  # Reduced from 0.9 to 0.5 (much less bouncy walls)
        }
        ...
        while self.accumulator >= fixed_dt:
            ...
            for i in range(len(self.floor_headsets)):
                for j in range(i + 1, len(self.floor_headsets)):
                    if self.floor_headsets[i].check_collision(self.floor_headsets[j]):
                        # Override the default COR in resolve_collision by passing in a much lower value
                        self.resolve_collision(self.floor_headsets[i], self.floor_headsets[j], 0.3)
            ...
            for headset in self.floor_headsets:
                ...
                if headset.position.y - headset.radius <= 0.01:
                    ...
                    if horizontal_speed_squared > 0.1:
                        # Apply stronger friction by applying coefficient twice
                        headset.velocity.x *= self.friction_coefficient * 0.95
                        headset.velocity.z *= self.friction_coefficient * 0.95
                    # Stop completely if very slow - significantly increased threshold
                    elif horizontal_speed_squared < 0.2:  # Increased threshold substantially
                ...
                if headset.position.x - headset.radius < boundary['min_x']:
                    ...
                    # Add smaller random variation to z velocity on x boundary collision
                    headset.velocity.z += (random.random() - 0.5) * 0.2  # Reduced from 0.5 to 0.2
                ...
                if headset.position.z - headset.radius < boundary['min_z']:
                    ...
                    # Add smaller random variation to x velocity on z boundary collision
                    headset.velocity.x += (random.random() - 0.5) * 0.2  # Reduced from 0.5 to 0.2
            ...
            # Only add minimal impulses very infrequently and early in the simulation
            if self.frame_count % 180 == 0 and self.frame_count < self.target_frames * 0.3:  # Even less frequent
                for headset in self.floor_headsets:
                    # Only add impulses to completely stopped headsets
                    speed_sq = headset.velocity.x**2 + headset.velocity.z**2
                    if speed_sq < 0.05:  # Only for essentially stopped objects
                        # Add very small random impulse
                        headset.velocity.x += (random.random() - 0.5) * 0.2  # Minimal impulse
                        headset.velocity.z += (random.random() - 0.5) * 0.2  # Minimal impulse
            
            self.accumulator -= fixed_dt

    def resolve_collision(self, obj1, obj2, cor=0.5):
        """
        Custom collision resolution with specified coefficient of restitution.
        This overrides the default value in the CollisionObject class.
        
        Args:
            obj1: First collision object
            obj2: Second collision object
            cor: Coefficient of restitution (0=inelastic, 1=perfectly elastic)
        """
        # Skip if we recently collided with this object (prevents sticking)
        if obj2 in obj1.recent_collisions:
            return
        ...
        # Calculate impulse using our specified coefficient of restitution
        j = -(1 + cor) * (relative_velocity * normal)
        j = j / 2  # Assuming equal masses for simplicity
        
        # Apply impulse to velocities
        obj1.velocity = obj1.velocity + (normal * j)
        obj2.velocity = obj2.velocity - (normal * j)
        
        # Add to recent collisions
        obj1.recent_collisions.add(obj2)
        obj2.recent_collisions.add(obj1)
        
        # Add slight position adjustment to prevent sticking
        # Move objects apart slightly along normal
